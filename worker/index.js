...
import {
    Router
} from 'itty-router'; // 如果你不想用外部 lib，直接使用 if/else 路由也可以
...
router.post('/api/employees/login', async (request, env) => {
    ...
    if (!body || !body.lineId) return new Response(JSON.stringify({
...
router.get('/api/beds', async (request, env) => {
    ...
    if (!employeeId) return new Response(JSON.stringify([]), {
        status: 200
    });
    ...
    if (!emp || emp.status !== 'enabled') return new Response(JSON.stringify([]), {
        status: 200
    });
...
router.get('/api/beds/:id/batches', async ({
    params
}, env) => {
    const id = params.id;
